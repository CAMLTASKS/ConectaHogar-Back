<?php
// created: 2026-05-25 18:11:29
$mod_strings['LBL_ACCOUNT_NAME'] = 'Conjunto';
$mod_strings['LBL_ACCOUNT_ID'] = 'ID de la cuenta:';
$mod_strings['LBL_ACCOUNT'] = 'Account';
$mod_strings['LNK_NEW_CONTACT'] = 'Create Residente';
$mod_strings['LNK_CONTACT_LIST'] = 'Residentes';
$mod_strings['LNK_IMPORT_VCARD'] = 'Importar Vcard';
$mod_strings['LNK_IMPORT_CONTACTS'] = 'Importar contactos';
$mod_strings['LBL_SAVE_CONTACT'] = 'Guardar contacto';
$mod_strings['LBL_LIST_FORM_TITLE'] = 'Residente List';
$mod_strings['LBL_SEARCH_FORM_TITLE'] = 'Búsqueda de contactos';
$mod_strings['LBL_REPORTS_TO'] = 'Reportar a:';
$mod_strings['LBL_DIRECT_REPORTS'] = 'Informadores directos';
$mod_strings['LBL_DIRECT_REPORTS_SUBPANEL_TITLE'] = 'Direct Reports';
$mod_strings['LBL_HOMEPAGE_TITLE'] = 'Mis contactos';
$mod_strings['LBL_MODULE_NAME'] = 'Residentes';
$mod_strings['LBL_MOBILE_PHONE'] = 'Celular principal';
$mod_strings['LBL_HOME_PHONE'] = 'Telefono fijo / interno';
$mod_strings['LBL_TIPO_RESIDENTE'] = 'Tipo de residente';
$mod_strings['LBL_ESTADO_CUENTA'] = 'Estado cuenta';
$mod_strings['LBL_CONTACT_INFORMATION'] = 'Datos personales';
$mod_strings['LBL_TITLE'] = 'Identificación';
$mod_strings['LBL_PANEL_ADVANCED'] = 'Datos de contacto';
$mod_strings['LBL_PANEL_ASSIGNMENT'] = 'Estado del sistema';
$mod_strings['LBL_LIST_ACCOUNT_NAME'] = 'Identificación';
$mod_strings['LBL_EMAIL_ADDRESS'] = 'Correo electrónico:';
$mod_strings['LBL_CAML_APARTAMENTOS_CONTACTS_1_FROM_CAML_APARTAMENTOS_TITLE'] = 'Apartamentos';
